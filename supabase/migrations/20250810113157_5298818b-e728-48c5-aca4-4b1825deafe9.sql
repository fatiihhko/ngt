...
-- RLS: only authenticated users may manage invites
create policy if not exists "Authenticated can read invites" on public.invites
for select using (auth.role() = 'authenticated');

create policy if not exists "Authenticated can insert invites" on public.invites
for insert with check (auth.role() = 'authenticated');

create policy if not exists "Authenticated can update invites" on public.invites
for update using (auth.role() = 'authenticated');

create policy if not exists "Authenticated can delete invites" on public.invites
